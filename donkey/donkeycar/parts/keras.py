...
class KerasCategorical(KerasPilot):
    ...
    def run(self, img_arr):
        ...
        if angle_certainty<0.1:
            angle_certainty=0.1
        new_angle=self.lastAngle*(1.0-angle_certainty)+angle_unbinned*angle_certainty
        self.lastAngle=new_angle
        if new_angle>=0.0:
                print('auto angle  %.1f throt %.1f confi %.1f' % (new_angle,throttle[0][0],angle_certainty))
        else:
                print('auto angle %.1f throt %.1f confi %.1f' % (new_angle,throttle[0][0],angle_certainty))
#        return angle_unbinned, throttle[0][0]
        return new_angle,throttle[0][0]
